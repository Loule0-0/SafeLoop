...
if [[ "${MODE}" != "baseline" && "${MODE}" != "monitor" && "${MODE}" != "rl" ]]; then
  echo "MODE must be baseline, monitor, or rl, got: ${MODE}" >&2
  exit 2
fi
...
command=(
  ...
  --post-rollback-action-noise-std "${POST_ROLLBACK_ACTION_NOISE_STD:-0.04}"
  --post-rollback-action-noise-final-scale "${POST_ROLLBACK_ACTION_NOISE_FINAL_SCALE:-0.25}"
  --post-rollback-action-noise-replans "${POST_ROLLBACK_ACTION_NOISE_REPLANS:-1}"
...
if [[ "${MODE}" == "baseline" ]]; then
  ...
  command+=(
    ...
    --record-probability "${RECORD_PROBABILITY:-0.14}"
    ...
    --rollback-cooldown "${ROLLBACK_COOLDOWN:-60}"
    --min-rollback-step "${MIN_ROLLBACK_STEP:-30}"
    ...
    --rollback-gate-current-body-threshold "${ROLLBACK_GATE_CURRENT_BODY_THRESHOLD:-0.55}"
    --rollback-gate-current-object-threshold "${ROLLBACK_GATE_CURRENT_OBJECT_THRESHOLD:-0.99}"
    --rollback-gate-future-body-probability "${ROLLBACK_GATE_FUTURE_BODY_PROBABILITY:-0.50}"
    --rollback-gate-future-object-probability "${ROLLBACK_GATE_FUTURE_OBJECT_PROBABILITY:-0.98}"
    --rollback-gate-max-current-object-probability "${ROLLBACK_GATE_MAX_CURRENT_OBJECT_PROBABILITY:-1.01}"
    --rollback-gate-future-tth "${ROLLBACK_GATE_FUTURE_TTH:-0.80}"
    --rollback-gate-future-object-tth "${ROLLBACK_GATE_FUTURE_OBJECT_TTH:-0.18}"
    --rollback-target-safe-score-threshold "${ROLLBACK_TARGET_SAFE_SCORE_THRESHOLD:-0.90}"
    --rollback-target-min-age "${ROLLBACK_TARGET_MIN_AGE:-30}"
    --rollback-target-max-age "${ROLLBACK_TARGET_MAX_AGE:-200}"
    --rollback-target-require-safe
    --auto-record-safe-anchors
    --auto-record-min-interval "${AUTO_RECORD_MIN_INTERVAL:-40}"
    --auto-record-max-risk-score "${AUTO_RECORD_MAX_RISK_SCORE:-0.90}"
    --auto-record-max-current-body-probability "${AUTO_RECORD_MAX_CURRENT_BODY_PROBABILITY:-0.44}"
    --auto-record-max-current-object-probability "${AUTO_RECORD_MAX_CURRENT_OBJECT_PROBABILITY:-0.90}"
  ...
  if [[ "${RECORD_INITIAL_SAFE_ANCHOR:-1}" == "1" ]]; then
    command+=(--record-initial-safe-anchor)
  elif [[ "${RECORD_INITIAL_SAFE_ANCHOR:-1}" != "0" ]]; then
    echo "RECORD_INITIAL_SAFE_ANCHOR must be 0 or 1, got: ${RECORD_INITIAL_SAFE_ANCHOR}" >&2
    exit 2
  fi
  if [[ -n "${INITIAL_ANCHOR_ROLLBACK_MIN_CURRENT_BODY_PROBABILITY:-0.30}" ]]; then
    command+=(
      --initial-anchor-rollback-min-current-body-probability
      "${INITIAL_ANCHOR_ROLLBACK_MIN_CURRENT_BODY_PROBABILITY:-0.30}"
    )
  fi
  if [[ -n "${INITIAL_ANCHOR_ROLLBACK_MIN_CURRENT_OBJECT_PROBABILITY:-0.99}" ]]; then
    command+=(
      --initial-anchor-rollback-min-current-object-probability
      "${INITIAL_ANCHOR_ROLLBACK_MIN_CURRENT_OBJECT_PROBABILITY:-0.99}"
    )
  fi
  if [[ "${ROLLBACK_TARGET_PREFER_RECENT_SAFE:-1}" == "1" ]]; then
    command+=(--rollback-target-prefer-recent-safe)
  elif [[ "${ROLLBACK_TARGET_PREFER_RECENT_SAFE:-1}" != "0" ]]; then
    echo "ROLLBACK_TARGET_PREFER_RECENT_SAFE must be 0 or 1, got: ${ROLLBACK_TARGET_PREFER_RECENT_SAFE}" >&2
    exit 2
  fi
fi
